...
// Helper function to convert optional string to optional path
...
void _SubsampleResultIO::_prepareSubsampleResultDir()
{
    if (_resultDir)
    {
        ...
        {
            throw std::runtime_error("_SubsampleResultIO::_prepareSubsampleResultDir: Filesystem error when creating directory: " +
                                     _resultDir->string() + ": " + e.what());
        }
        catch (const std::exception &e)
        {
            throw std::runtime_error("_SubsampleResultIO::_prepareSubsampleResultDir: Error when creating directory: " +
                                     _resultDir->string() + ": " + e.what());
        }
    }
}
...
void _SubsampleResultIO::_dumpSubsampleResult(const Result &learningResult, int index)
{
    ...
    {
        throw std::runtime_error("_SubsampleResultIO::_dumpSubsampleResult: Error serializing learning result for index " +
                                 std::to_string(index) + " via BaseLearner: " + e.what());
    }
    ...
    if (ZSTD_isError(cSize))
        throw std::runtime_error("_SubsampleResultIO::_dumpSubsampleResult: ZSTD compression error for index " +
                                 std::to_string(index) + ": " + ZSTD_getErrorName(cSize));
...
Result _SubsampleResultIO::_loadSubsampleResult(int index)
{
    ...
    {
        inFile.close();
        throw std::runtime_error("_SubsampleResultIO::_loadSubsampleResult: Failed to read compressed data from file: " +
                                 resultPath.string());
    }
    ...
    if (bufferSize == ZSTD_CONTENTSIZE_ERROR)
        throw std::runtime_error("_SubsampleResultIO::_loadSubsampleResult: File is not a valid ZSTD compressed file: " +
                                 resultPath.string());
    if (bufferSize == ZSTD_CONTENTSIZE_UNKNOWN)
        throw std::runtime_error("_SubsampleResultIO::_loadSubsampleResult: Unknown decompressed size for file: " +
                                 resultPath.string());
    ...
    if (ZSTD_isError(decompressedSize))
        throw std::runtime_error("_SubsampleResultIO::_loadSubsampleResult: ZSTD decompression error for index: " +
                                 std::to_string(index) + ": " + ZSTD_getErrorName(decompressedSize));
    if (decompressedSize != bufferSize)
        throw std::runtime_error("_SubsampleResultIO::_loadSubsampleResult: Zstd decompression size mismatch for index: " +
                                 std::to_string(index) + ": expected " + std::to_string(bufferSize) + ", got " + std::to_string(decompressedSize));
    ...
    {
        throw std::runtime_error("_SubsampleResultIO::_loadSubsampleResult: Error deserializing learning result for index " +
                                 std::to_string(index) + " via BaseLearner: " + e.what());
    }
